...
import com.pig4cloud.pig.common.core.entity.manager.Define;
...
import com.pig4cloud.pig.monitor.service.ObjectStoreService;
...
public class AppServiceImpl implements AppService, InitializingBean {
    ...
    private static final List<String> huaweiSwitchMetricsBlack = Arrays.asList("mtu", "speed", "in_octets", "in_discards");
    private static final List<String> huaweiSwitchMetricsContains = Arrays.asList();
    ...
    @Override
    public Job getAppDefineCustom(String app) throws IllegalArgumentException {
        if (StringUtils.isBlank(app)) {
            throw new IllegalArgumentException("The app can not null.");
        }
        var appDefine = appDefines.get(app.toLowerCase());
        ...
        if ("huawei_switch".equals(app)) {
            List<Metrics> metrics = appDefine.getMetrics();
//            List<Metrics> newMetrics = metrics.stream().filter(metric -> !"interfaces".equals(metric.getName()))
//                .collect(Collectors.toList());
//            appDefine.setMetrics(newMetrics);
            for (Metrics metric : metrics) {
                if ("interfaces".equals(metric.getName())) {
                    List<Metrics.Field> fields = metric.getFields();
                    List<Metrics.Field> collect = fields.stream().filter(j -> huaweiSwitchMetricsContains.contains(j.getField())).collect(Collectors.toList());
                    metric.setFields(collect);
                }
            }
        }
        return appDefine.clone();
