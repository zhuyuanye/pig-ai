>将 Zabbix 的告警通过 Webhook 方式发送到告警平台。
...
try {
  ...
  // 记录日志
  Zabbix.Log(4, "webhook payload: " + JSON.stringify(hertzbeatAlert));
...
3. 配置操作选项卡信息
    - 操作步骤持续时间: 根据需要进行设置
    - 在**操作**部分添加，配置用户或用户组，选择之前配置 Webhook 媒介的用户，发送至媒体类型选择 ** Webhook**，选中自定义消息内容，确保所有宏都被正确传递
