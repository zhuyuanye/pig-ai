...
/**
 * 告警规则定义批量操作控制器
 * <p>提供告警规则的批量查询、批量删除、导入导出等操作接口</p>
 *
 * @author pig4cloud
 */
...
public class AlertDefinesController {
    ...
    /**
     * 根据查询过滤条件获取告警规则定义列表（分页）
     *
     * @param ids       告警规则定义ID列表
     * @param search    搜索关键字
    ...
    public ResponseEntity<Message<IPage<AlertDefine>>> getAlertDefines(
        ...
        IPage<AlertDefine> alertDefinePage = alertDefineService.getAlertDefines(ids, search, sort, order, pageIndex, pageSize);
        return ResponseEntity.ok(Message.success(alertDefinePage));
    }

    /**
     * 根据告警规则定义ID列表批量删除告警规则
     *
     * @param ids 告警规则定义ID列表
     * @return 操作结果
     */
     ...
     *
     * @param ids  告警规则定义ID列表
     * @param type 导出类型：JSON、EXCEL、YAML
     * @param res  HTTP响应对象
