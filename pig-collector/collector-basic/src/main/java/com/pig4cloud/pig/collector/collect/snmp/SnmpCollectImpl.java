...
package com.pig4cloud.pig.collector.collect.snmp;

import lombok.extern.slf4j.Slf4j;
import com.pig4cloud.pig.collector.collect.AbstractCollect;
import com.pig4cloud.pig.collector.constants.CollectorConstants;
...
import com.pig4cloud.pig.common.core.entity.job.protocol.SnmpProtocol;
    ...
    public void collect(CollectRep.MetricsData.Builder builder, Metrics metrics) {
        ...
        try {
            ...
            if (OPERATION_GET.equalsIgnoreCase(operation)) {
                ...
                // 假设 tableEvents 是需要分割的 List<TableEvent>
                int size = tableEvents.size();
                int mid = size / 2;

                // 分割成两部分
                List<TableEvent> firstHalf = tableEvents.subList(0, mid);
                List<TableEvent> secondHalf = tableEvents.subList(mid, size);
                int secondHalfSize = secondHalf.size();

                Assert.notNull(tableEvents, "snmp walk response empty content.");
                long responseTime = System.currentTimeMillis() - startTime;
                for (int i = 0; i < firstHalf.size(); i++) {
                    TableEvent tableEvent = firstHalf.get(i);
                    if (tableEvent == null || tableEvent.isError()) {
                        continue;
                    }
                    if (i > secondHalfSize - 1) {
                        break;
                    }

                    TableEvent secondHalfTableEvent = secondHalf.get(i);
                    VariableBinding[] columns = tableEvent.getColumns();
                    VariableBinding[] secondHalfColumns = secondHalfTableEvent.getColumns();

                    // 二合一
                    VariableBinding[] varBindings = new VariableBinding[columns.length + secondHalfColumns.length];
                    System.arraycopy(columns, 0, varBindings, 0, columns.length);
                    System.arraycopy(secondHalfColumns, 0, varBindings, columns.length, secondHalfColumns.length);
